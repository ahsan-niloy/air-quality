...
export default function Profile() {
  ...
  const initials = useMemo(() => initialsFrom(user?.displayName || user?.email), [user]);

  // Replace with your real auth actions:
  const handleSave = () => {
    // Example only — replace with your update profile call
    console.log("Save profile:", { displayName: draftName, photoURL: draftPhoto });
    setOpen(false);
  };
  const handleSignOut = () => {
    signOutUser()
      ...
      <div className="flex items-center justify-center p-6">
        <Card className="w-full max-w-xl">
          ...
          <CardFooter className="justify-end">
            <Button onClick={() => console.log("TODO: sign in")}>Sign in</Button>
          </CardFooter>
        </Card>
      </div>
    );
